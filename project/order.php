...
session_start();
$cart_summary = "";
if (isset($_SESSION['cart_summary']) && !empty($_SESSION['cart_summary'])) {
$products_list = [
        1 => "Κουραμπιέδες",
        2 => "Μελομακάρονα",
        3 => "Εκλεράκια",
...
foreach ($_SESSION['cart_summary'] as $id => $qty) {
...
$email     = $_POST['email'];
 ...
 if ($success){
	 echo "<h2>Η παραγγελία σας καταχωρήθηκε επιτυχώς!</h2>";
        echo "<p>Σύντομα θα λάβετε email επιβεβαίωσης ".htmlspecialchars($email).".</p>";
		unset($_SESSION['cart']);
		} else {
    echo "Σφάλμα κατά την αποθήκευση: " . mysqli_error($dblink);
}
